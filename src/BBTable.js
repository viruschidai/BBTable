...
(function() {
  ...
  var Cell = BBLib.Cell = Backbone.View.extend({
    ...
    events: {
      "click, dbclick": function(e) {
        this.trigger(event, this.model, this.col);
      }
    },
  ...
  var HeaderCell = BBLib.HeaderCell = Backbone.View.extend({
    ...
    sortingTemplate: _.template("<div class='sorting sorting-<%= sortDir %> pull-right'><i class='<%= iconClass %>'></i></div>"),
  ...
  $.fn.bbtable = function(options) {
    requiredOptions(['url', 'columns'])

    options = $.extend({
      modelCls: Backbone.Model,
    ...
    return this.each(function() {
      ...
      var cols = new Columns(options.columns),
          ...
          Collection = pageableCls.extend({url: options.url, model: options.modelCls}),
